    ...
    <section className="w-full bg-white pb-14">
      ...
      <div className="w-11/12 mx-auto flex flex-wrap justify-center gap-6">
        {steps.map((step, index) => (
          <Slide direction="left" key={step.id}  cascade damping={0.5}>
          <div className="flex items-center gap-3">
            <div className="bg-[#dff2fe] w-[200px] flex items-center justify-center flex-col gap-2 py-6 px-6 rounded-xl shadow-xl hover:bg-blue-950 hover:text-white transition-all duration-300 ease-in-out group min-w-[180px]">
              <div className="text-black group-hover:text-white">
                {step.icon}
              </div>
              <h4 className="text-black text-center text-sm md:text-base group-hover:text-white">
                {step.title}
              </h4>
            </div>
